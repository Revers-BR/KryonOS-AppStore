...
function drawMenu() {
    ...
    Display.drawString("HARIX BENCHMARK", 20, 20, 4);

    Display.setTextColor(0xFFFF, 0x0000);
    Display.drawString("Select a stress test:", 20, 60, 2);

    // CPU Button
    Display.fillRoundRect(30, 90, 180, 45, 5, 0xF800);
    Display.setTextColor(0xFFFF, 0xF800);
    Display.drawString("CPU & MATH", 70, 105, 2);

    // 2D Button
    Display.fillRoundRect(30, 150, 180, 45, 5, 0x07E0);
    Display.setTextColor(0x0000, 0x07E0);
    Display.drawString("2D GRAPHICS", 65, 165, 2);

    // 3D Button
    Display.fillRoundRect(30, 210, 180, 45, 5, 0x001F);
    Display.setTextColor(0xFFFF, 0x001F);
    Display.drawString("3D GRAPHICS", 65, 225, 2);
}

function updateTemps() {
    if (Harix.hasTemperatureSensor()) {
        var t = Harix.getTemperature();
        if (t < minT) minT = t;
        if (t > maxT) maxT = t;
        return t;
    }
...
function runCPU(durationMs) {
    var start = Harix.millis();
    var ops = 0;
    var runningVal = 1.0;

    Display.fillScreen(0x0000);
    Display.setTextColor(0x07FF, 0x0000);
    Display.drawString("CPU BENCHMARK RUNNING", 10, 100, 2);

    while (Harix.millis() - start < durationMs) {
        var chunkStart = Harix.millis();
        var tempOps = 0;

        while (Harix.millis() - chunkStart < 50) {
        ...
        var t = updateTemps();
        Display.fillRect(10, 140, 220, 30, 0);
        Display.setTextColor(0xFFFF, 0x0000);
        Display.drawString("Ops: " + ops + " Temp: " + Math.floor(t) + "C", 10, 140, 2);
        Display.drawString("Val: " + runningVal.toFixed(2), 10, 160, 2);
        Harix.delay(1); // Watchdog
    }
...
function run2D(durationMs) {
    ...
    while (Harix.millis() - start < durationMs) {
        ...
        if (shape === 0) Display.fillRect(x1, y1, w, h, c);
        else if (shape === 1) Display.fillCircle(x1, y1, w / 2, c);
        else if (shape === 2) Display.drawLine(x1, y1, x1 + w, y1 + h, c);
        else if (shape === 3) Display.fillRoundRect(x1, y1, w, h, 10, c);

        frames++;

        if (frames % 20 === 0) {
            var t = updateTemps();
            Display.fillRect(0, 0, SW, 20, 0x0000);
            Display.setTextColor(0xFFFF, 0x0000);
            Display.drawString("2D Primitives: " + frames + " Temp: " + Math.floor(t) + "C", 5, 2, 2);
            Harix.delay(1);
        }
    }
...
function run3D(durationMs) {
    ...
    var lastFrameTime = Harix.millis();
    var fps = 0;
    ...
    while (Harix.millis() - start < durationMs) {
        ...
        for (var slice = 0; slice < numSlices; slice++) {
            ...
            Sprite.bind(true);
            Display.fillScreen(0x0000); // Clear slice buffer
            ...
            for (var f = 0; f < 6; f++) {
                ...
                if (maxY >= sliceY && minY < sliceY + sliceH) {
                    ...
                    Display.fillTriangle(p0.px, p0.py - sliceY, p1.px, p1.py - sliceY, p2.px, p2.py - sliceY, face.color);
                    Display.fillTriangle(p0.px, p0.py - sliceY, p2.px, p2.py - sliceY, p3.px, p3.py - sliceY, face.color);
                }
            }
            ...
            if (slice === 0) {
                var t = updateTemps();
                Display.setTextColor(0xFFFF, 0x0000);
                Display.drawString("3D FPS: " + fps + " | Temp: " + Math.floor(t) + "C", 5, 5, 2);
            }
...
function drawResult() {
    ...
    Display.drawString("BENCHMARK COMPLETE", 10, 30, 4);

    Display.setTextColor(0xFFFF, 0x0000);
    if (selectedBench === 0) Display.drawString("Type: CPU & MATH", 20, 80, 2);
    else if (selectedBench === 1) Display.drawString("Type: 2D GRAPHICS", 20, 80, 2);
    else if (selectedBench === 2) Display.drawString("Type: 3D GRAPHICS", 20, 80, 2);

    Display.setTextColor(0xF800, 0x0000);
    Display.drawString("SCORE:", 20, 120, 4);

    Display.setTextColor(0x07FF, 0x0000);
    Display.drawString(resultScore + " " + resultText, 20, 150, 4);

    if (minT !== 999) {
        Display.setTextColor(0xFFFF, 0x0000);
        Display.drawString("Thermal Impact:", 20, 210, 2);
        Display.setTextColor(0x001F, 0x0000);
        Display.drawString("Min Temp: " + Math.floor(minT) + " C", 20, 230, 2);
        Display.setTextColor(0xF800, 0x0000);
        Display.drawString("Max Temp: " + Math.floor(maxT) + " C", 20, 250, 2);
    }

    Display.setTextColor(0x7BEF, 0x0000);
    Display.drawString("Tap to return", 60, 290, 2);
}
    ...
    if (isTapped && t.x < SW - 40) { // Avoid OS close button area
        if (state === STATE_MENU) {
            ...
            if (selected !== -1) {
                ...
                var timeStr = Keyboard.prompt("Enter duration in seconds (1-999):", "10");
